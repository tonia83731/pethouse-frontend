...
import { INTABLEHEADER, intablekeys } from "../statistic/TABLEHEADER";
const StatisticInTable = ({
  bodyData,
  totalPage,
}: {
  bodyData: any[];
  ...
  const columns = intablekeys.map((key: string) =>
    columnHelper.accessor(key, {
      header: () => `${INTABLEHEADER.get(key)}`,
  ...
  return (
    <div className="flex flex-col gap-4">
      <h4 className="font-extrabold font-nunito text-2xl">入所方式</h4>
      <table className="">
        <thead className="rounded-md md:text-lg font-bold bg-milk-tea text-dark inline-block w-full">
          {table.getHeaderGroups().map((headerGroup) => {
            return (
              <tr
                className="rounded-md grid grid-cols-10 items-center"
                key={headerGroup.id}
              >
          ...
          <>
            {table.getRowModel().rows.map((row) => {
              return (
                <tr
                  key={row.id}
                  className={`border-b border-wine border-dotted last:border-none py-[20px] grid grid-cols-10 w-[calc(100%+12px)]`}
                >
...
export default StatisticInTable;
